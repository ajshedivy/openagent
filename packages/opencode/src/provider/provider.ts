...
export namespace Provider {
  ...
  const BUNDLED_PROVIDERS: Record<string, (options: any) => SDK> = {
    ...
    // @ts-ignore - AgentOS provider only implements languageModel
    "@opencode/agentos": createAgentOS,
  }
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    agentos: async (input) => {
      ...
      const config = await Config.get()
      const providerConfig = config.provider?.["agentos"]
      ...
      const baseURL =
        ...
        (providerConfig?.api as string | undefined) ||
  ...
  const state = Instance.state(async () => {
    ...
    for (const plugin of await Plugin.list()) {
      ...
      // Special handling for agentos: run loader if baseURL is configured (auth is optional)
      if (providerID === "agentos" && !hasAuth) {
        const agentosBaseURL =
          config.provider?.["agentos"]?.options?.baseURL ||
          config.provider?.["agentos"]?.api ||
          Env.get("AGENTOS_API_URL")
        if (agentosBaseURL) hasAuth = true
      }

      if (!hasAuth) continue
      if (!plugin.auth.loader) continue

      // Load for the main provider if auth exists (or for agentos which doesn't require auth)
      if (auth || providerID === "agentos") {
