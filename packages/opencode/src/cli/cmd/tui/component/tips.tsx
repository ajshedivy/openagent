...
const TIPS = [
  ...
  "Create {highlight}openagent.json{/highlight} in project root for project-specific settings",
  "Place settings in {highlight}~/.config/openagent/openagent.json{/highlight} for global config",
  ...
  "openagent auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.openagent/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.openagent/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "openagent auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "openagent uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.openagent/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.openagent/plugin/{/highlight} for event hooks",
  ...
  "Use {highlight}openagent --continue{/highlight} to resume the last session",
  "Use {highlight}openagent run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}openagent serve{/highlight} for headless API access to openagent",
  "Use {highlight}openagent run --attach{/highlight} to connect to a running server",
  "Run {highlight}openagent upgrade{/highlight} to update to the latest version",
  "Run {highlight}openagent auth list{/highlight} to see all configured providers",
  "Run {highlight}openagent agent create{/highlight} for guided agent creation",
  "Use {highlight}/openagent{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}openagent github install{/highlight} to set up the GitHub workflow",
  ...
  "Run {highlight}openagent debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/openagent{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with openagent Zen for curated, tested models",
