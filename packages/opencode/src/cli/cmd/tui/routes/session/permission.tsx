...
function AgentOSToolBody(props: { request: PermissionRequest }) {
  const { theme } = useTheme()
  const toolName = () => (props.request.metadata?.tool_name as string) ?? "Unknown Tool"
  const toolArgs = () => props.request.metadata?.tool_args as Record<string, unknown> | undefined

  const argsDisplay = createMemo(() => {
    const args = toolArgs()
  ...
  return (
    <box flexDirection="column" gap={1}>
      <box flexDirection="row" gap={1} paddingLeft={1}>
        <text fg={theme.textMuted}>⚙</text>
        <text fg={theme.text}>
          AgentOS Tool: <span style={{ fg: theme.primary }}>{toolName()}</span>
        </text>
      </box>
      <Show when={argsDisplay()}>
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Arguments:</text>
        </box>
        <scrollbox height={10} paddingLeft={2}>
          <text fg={theme.text}>{argsDisplay()}</text>
        </scrollbox>
      </Show>
    </box>
  )
          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"This will allow " + props.request.permission + " until openagent is restarted."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until openagent is restarted</text>
                  <box>
            ...
            <Prompt
              ...
              body={
                <Switch>
                  ...
                  <Match when={props.request.permission === "agentos_tool"}>
                    <AgentOSToolBody request={props.request} />
                  </Match>
                  <Match when={true}>
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell openagent what to do differently</text>
        </box>
      </box>
      <box
