# Requirements Archive: v2.0 AgentOS SDK Migration
...
- [x] **STRM-01**: `client.agents.runStream()` replaces custom `makeStreamingRequest()` with FormData
- [x] **STRM-02**: SDK `AgentStream` events mapped to AI SDK `LanguageModelV2StreamPart` interface
- [x] **STRM-03**: Custom `createSSEParser()` TransformStream removed entirely
- [x] **STRM-04**: Non-streaming path uses `client.agents.run()` instead of custom `makeNonStreamingRequest()`
- [x] **STRM-05**: `RunPaused` event from `AgentStream` correctly triggers tool confirmation workflow
- [x] **STRM-06**: `RunCompleted` event properly signals stream end with usage metadata

### Tool Confirmation & Continue

- [x] **TOOL-01**: `client.agents.continue()` replaces custom `makeContinueRequest()` with FormData
- [x] **TOOL-02**: Continue response stream processed via SDK `AgentStream` (replaces `processContinueStream()`)
- [x] **TOOL-03**: Paused state (runId, sessionId, requirements) handled using SDK types
- [x] **TOOL-04**: Tool approval/rejection flow works end-to-end through SDK

### Run Lifecycle

- [x] **RUN-01**: `client.agents.cancel()` wired up for run cancellation
- [x] **RUN-02**: Provider factory (`createAgentOS`) returns SDK-backed language model
- [x] **RUN-03**: Full agent chat workflow verified: discover -> connect -> stream -> tool confirm -> continue/cancel

### Type Cleanup

- [x] **TYPE-01**: Custom Zod schemas in `agentos-types.ts` removed (replaced by SDK types)
- [x] **TYPE-02**: SDK types re-exported where needed by other modules (plugin, processor, UI)
- [x] **TYPE-03**: No hand-written AgentOS API types remain in codebase
